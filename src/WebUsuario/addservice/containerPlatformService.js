...
const getContainerStyle = (id) => {
  ...
  return (
    <div className="container-platform-service" style={{ backgroundColor, color }}>
      <h2 className={`servicio-title ${titleClass}`} style={{ color: titleColor }}><strong></strong> {formatId(id)}</h2>
      <p className="precio-text" style={{ color: priceColor }}><strong>Precio:</strong> {formatPrice(precio)}</p>
      ...
      {isModalOpen && (
        <div className="overlay">
          <div className="mensaje-container">
            ...
            <ul>{formatInfoList(info)}</ul> {/* Aquí mostramos la lista de información */}
